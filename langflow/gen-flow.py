...
NODES: list[dict] = [
    ...
    dict(
        ...
        role=(
            ...
            "re-runs the pattern half, its tests, the drift gate and the build, then opens the PR. On that PR "
            "gates.yml only queues and waits for a maintainer approval (action_required, observed 09-11 to 09-15) "
            "because the pusher is the Actions bot; the same gates ran inside the job; gates.yml runs unattended "
            "on main after the merge. Both cadences in the title are READ from the "
